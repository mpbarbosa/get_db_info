...
class DatabaseConn:
    ...
    def __init__(self,user:str,password:str,host:str,port:str='1521',service_name:str=None,conn_method=None) -> None:
        self.user = user
    ...
    def execute_sql(self,final_sql_query:str=None,parameters:Union[list,None]=None):
        return self._execute(final_sql_query=final_sql_query,parameters=parameters)
    
    def get_sql_result_as_data_frame(self,final_sql_query:str=None,parameters:Union[list,None]=None):
        terminal.print_debug(txt=f'Inicio da funcao OracleConn.get_sql_result_as_data_frame(final_sql_query={final_sql_query},parameters={parameters}')
        self.connect()
        exec_result = self.execute_sql(final_sql_query=final_sql_query,parameters=parameters)
        terminal.print_debug(txt=f'exec_result={exec_result}')
        result = self.dbcursor().fetchall()
        return pd.DataFrame(result, columns=[col[0] for col in self.dbcursor().description])
    
    def fetchone(self,final_sql_query:str=None,parameters:Union[list,None]=None):
        terminal.print_debug(txt=f'Inicio da funcao DatabaseConn.get_sql_result_as_data_frame(final_sql_query={final_sql_query},parameters={parameters}')
        self.connect()
        exec_result = self.execute_sql(final_sql_query=final_sql_query,parameters=parameters)
        terminal.print_debug(txt=f'exec_result={exec_result}')
...
class OracleConn(DatabaseConn):
    ...
    def connect(self):  
        ...
        if (self.conn_method == 'oracledb'):
            terminal.print_debug(f"Comecando criacao da conexao Oracle com metodo: {self.conn_method}") 
            try:
                self.conn = self._connect()
            except oracledb.Error as error:
                error_msg = str(error)
                terminal.print_error(f'Error connecting to Oracle:' + error_msg,log_lvl=0)
                if ('DPY-3015' in error_msg):
                    ...
                    terminal.print_error('Nao foi possivel conectar com o banco de dados oracle: {dsn}')
                    ...
                    terminal.print_error(f'Nao foi possivel conectar com o banco de dados oracle: {self.get_dsn()}')
                    terminal.print_warning("Saindo...")
                    raise error
        ...
        terminal.print_debug(f"Retornando conexao Oracle: {self.conn}, type: {type(self.conn)}") 
        return self.conn
...
class PostgresqlConn(DatabaseConn):
    ...
    def _connect(self):
        ...
        terminal.print_monitor(f'Connected to Postgresql Database: {self.conn}, type: {type(self.conn)}!')
        return self.conn
    ...
    def connect(self):  
        ...
        if (self.conn_method == 'psycopg2'):
            terminal.print_debug(f"Comecando criacao da conexao Postgresql com metodo: {self.conn_method}") 
            try:
                self.conn = self._connect()
            ...
            terminal.print_debug(f"Connecting PostgreSQL Database using sqlalchemy: ...") 
            connection_url = self.get_get_conn_url()
            self.engine = create_engine(connection_url)
            self.conn = self.engine.connect()
        ...
        terminal.print_debug(f"Retornando conexao PostgreSQL: {self.conn}, type: {type(self.conn)}") 
        return self.conn
